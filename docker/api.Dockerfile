# Device Tracking Inventory System - API Dockerfile
...
COPY ["DeviceTracking.Inventory.Api/DeviceTracking.Inventory.Api.csproj", "DeviceTracking.Inventory.Api/"]
...
RUN dotnet restore "DeviceTracking.Inventory.Api/DeviceTracking.Inventory.Api.csproj"

# Copy source code
COPY ["DeviceTracking.Inventory.Api/", "DeviceTracking.Inventory.Api/"]
...
# Build application
WORKDIR "/src/DeviceTracking.Inventory.Api"
RUN dotnet build "DeviceTracking.Inventory.Api.csproj" -c Release -o /app/build

# Publish stage
FROM build AS publish
RUN dotnet publish "DeviceTracking.Inventory.Api.csproj" -c Release -o /app/publish /p:UseAppHost=false
...
# Entry point
ENTRYPOINT ["dotnet", "DeviceTracking.Inventory.Api.dll"]
